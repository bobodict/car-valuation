<template>
  <section class="panel evidence-panel" aria-labelledby="evidence-title">
    ...
    <div class="evidence-grid">
      <div class="chart-wrap"><div class="chart-heading"><div><span class="eyebrow">ERROR COMPARISON</span><strong>模型 vs 均值基线</strong></div><span>lower is better</span></div><div v-if="hasCompleteErrorMetrics" class="chart-canvas"><canvas ref="chartCanvas" aria-label="模型与均值基线 RMSE 对比图"></canvas></div><p v-else class="empty-evidence">暂无完整误差对比数据。</p></div>
...
const testMetrics = computed(() => props.metrics?.test_metrics ?? props.metrics ?? {})
const hasCompleteErrorMetrics = computed(() => Number.isFinite(testMetrics.value?.rmse) && Number.isFinite(testMetrics.value?.baseline_rmse))

function renderChart() {
  chart?.destroy()
  chart = null
  if (!chartCanvas.value || !hasCompleteErrorMetrics.value) return
  const test = testMetrics.value
  chart = new Chart(chartCanvas.value, {
    type: 'bar',
    data: { labels: ['模型 RMSE', '均值基线 RMSE'], datasets: [{ data: [test.rmse, test.baseline_rmse], backgroundColor: ['#2f7d5b', '#c8d3cc'], borderRadius: 4, barThickness: 28 }] },
