...
它不是只提供一个“输入车辆、输出价格”的页面，而是把模型选择、独立测试、误差分组、数据来源、特征契约和估值结果放在同一个可检查的研究控制台中，适合课程设计、机器学习工程实践和研究生复试项目展示。

## 项目亮点

- **Research Console 研究总览**：默认展示模型版本、质量门禁、开发集交叉验证、独立测试集、候选模型排行榜、误差分组、特征说明和数据来源。
- **可运行的车辆估值**：输入车辆品牌、车型、城市、里程、上牌年份、排量等字段，调用当前正式发布模型返回 INR 价格、参考区间和模型指标。
...
- **可追溯的历史记录**：每次估值会记录车辆摘要、价格、模型版本和时间，便于回看实验。
- **可选解释助手**：使用本地知识库和 OpenAI-compatible 接口；未配置 LLM 时明确返回禁用状态，不生成脱离数据的答案。
...
训练产生的原始数据、标准化数据和 experiments 目录默认位于 `backend/data/raw/`、`backend/data/processed/` 和 `backend/experiments/`，这些内容不提交到 Git。课程提交目录 `submit` 也不属于 GitHub 源码仓库。
...
```powershell
cd frontend
node --test src/researchEvidence.test.js
npm run build
```

当前验证结果：后端 `295 passed, 4 skipped`；前端研究证据测试 3 项通过，生产构建通过。
...
本仓库主要用于课程设计、研究展示和工程实践。公开数据的使用请遵循原始数据集发布者的许可和平台规则。仓库不提交数据库密码、LLM 密钥、原始训练数据或课程提交目录。
